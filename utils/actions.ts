...
export const createProfileAction = async (
  ...
  try {
    ...
    const validatedFiels = validateWithZodSchema(profileSchema, rawData);
...
export const fecthProfileImage = async () => {
  const user = await currentUser();
  if (!user) return null;
...
export const updateProfileImageAction = async (
  ...
  try {
    ...
    const validatedFiels = validateWithZodSchema(imageSchema, { image });
    const fullPath = await uploadImage(validatedFiels.image);
    await db.profile.update({
      where: {
        clerkId: user.id,
      },
...
export const createReviewAction = async (
  ...
  try {
    ...
    await revalidatePath(`/properties/${validatedFiels.propertyId}`);
...
export async function fecthPropertyRating(propertyId: string) {
...
export const findExitstingReview = async (
  userId: string,
  propertyId: string
) => {
  ...
  const rentalsWithBookingSums = await Promise.all(
    rentals.map(async (rental) => {
      ...
      return {
        ...
        orderTotalSum: orderTotal._sum.orderTotal || 0,
      };
    })
  );
...
export const deleteRentalAction = async (prevState: { propertyId: string }) => {
  ...
  try {
    ...
    return { message: "deleted rental successfully" };
  } catch (error) {
    return renderError(error);
  }
};
